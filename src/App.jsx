...
// Disguise mode: swap real (story-revealing) titles for a stable, believable
// email subject. Deterministic per text so a given item always shows the same one.
const FAKE_SUBJECTS = [
  "RE: Weekly status update", "FW: Meeting notes", "Q3 planning draft",
  "RE: Budget review", "Action items — follow-up", "RE: Project timeline",
  "Notes from this morning's standup", "FW: Policy update", "RE: Onboarding docs",
  "Draft: proposal for review", "RE: Client feedback", "FW: Schedule change",
  "RE: Expense report", "Team sync recap", "RE: Access request",
  "Following up on our chat", "RE: Next steps", "FW: Reminder — deadline Friday",
];
function fakeSubject(seed) {
  const s = String(seed || "");
  let h = 0;
  for (let i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) >>> 0;
  return FAKE_SUBJECTS[h % FAKE_SUBJECTS.length];
}
...
export default function StoryOrganizer() {
  ...
  const [draggingSubId, setDraggingSubId] = useState(null); // sub-note being dragged to reorder
  ...
  const [disguised, setDisguised] = useState(() => localStorage.getItem('qwosid_disguise') === '1');
  const [disguiseDark, setDisguiseDark] = useState(() => localStorage.getItem('qwosid_disguiseDark') === '1');
  ...
  useEffect(() => {
    localDB.load().then(({ stories: saved, currentStoryId: savedId, folders: savedFolders }) => {
      if (saved && saved.length > 0) {
        ...
        // One-time migration: move each character's linked note group + old
        // outline blocks into per-character noteSubs/outlineSubs. No text is lost.
        const migrated = purged.map(s => {
          let notes = s.notes || [];
          const removeNoteIds = new Set();
          const characters = (s.characters || []).map(c => {
            let nc = c;
            if (c.noteSubs === undefined) {
              const linked = notes.find(n => n.id === c.noteId);
              if (linked) removeNoteIds.add(linked.id);
              nc = { ...nc, noteSubs: linked ? (linked.subnotes || []).map(sn => ({ ...sn })) : [], noteId: null };
            }
            if (nc.outlineSubs === undefined) {
              nc = { ...nc, outlineSubs: (c.outline || []).map(b => {
                const text = b.text || "";
                const firstLine = text.trim().split("\n")[0].slice(0, 60);
                return { id: b.id || uid(), title: firstLine || "Untitled", content: text };
              }) };
            }
            return nc;
          });
          if (removeNoteIds.size === 0 && characters.every((c, i) => c === (s.characters || [])[i])) return s;
          return { ...s, characters, notes: notes.filter(n => !removeNoteIds.has(n.id)) };
        });
        setStories(migrated);
        if (savedId && migrated.find(s => s.id === savedId)) {
  ...
  // Disguise ("work mode"): flip the whole app to an Outlook-looking theme.
  // Purely visual — no data or behaviour changes. Toggle with Ctrl+Shift+O.
  useEffect(() => {
    document.documentElement.classList.toggle('disguise', disguised);
    document.documentElement.classList.toggle('disguise-dark', disguised && disguiseDark);
    try {
      localStorage.setItem('qwosid_disguise', disguised ? '1' : '0');
      localStorage.setItem('qwosid_disguiseDark', disguiseDark ? '1' : '0');
    } catch { /* visual only */ }
    document.title = disguised ? 'Inbox - Outlook' : 'Qwosid';
  }, [disguised, disguiseDark]);
  ...
  // Relationships hold a list of member character ids. Old data used charA/charB —
  // fall back to that so existing relationships keep working.
  const relMembers = r => (r.members && r.members.length ? r.members : [r.charA, r.charB].filter(Boolean));
  const relLabel = r => {
    const names = relMembers(r).map(id => charMap[id]?.name || "?");
    return names.length ? names.join(" & ") : "Relationship";
  };

  function relsFor(charId) {
    return currentStory ? currentStory.relationships.filter(r => relMembers(r).includes(charId)) : [];
  }
  ...
  function deleteItem(collection, id) {
    ...
    setStories(stories.map(s => {
      ...
      let nextRels = s.relationships;
      if (collection === "characters") {
        nextRels = [];
        for (const r of s.relationships) {
          const members = relMembers(r);
          if (!members.includes(id)) { nextRels.push(r); continue; }
          const remaining = members.filter(m => m !== id);
          if (remaining.length >= 2) {
            nextRels.push({ ...r, members: remaining }); // just drop this person
          } else {
            trashed.push({ collection: "relationships", item: r, deletedAt: now }); // <2 left → trash it
          }
        }
      }
      return {
        ...
        ...(collection === "characters" ? { relationships: nextRels } : {}),
  ...
  // Drag-reorder sub-notes within a note/outline group (drop fromId before toId)
  function reorderSubNote(noteId, fromId, toId, collKey = "notes") {
    if (!fromId || !toId || fromId === toId) return;
    setStories(stories.map(s => {
      if (s.id !== currentStoryId) return s;
      return { ...s, [collKey]: (s[collKey] || []).map(n => {
        if (n.id !== noteId) return n;
        const arr = [...(n.subnotes || [])];
        const from = arr.findIndex(x => x.id === fromId);
        const to = arr.findIndex(x => x.id === toId);
        if (from === -1 || to === -1) return n;
        const [moved] = arr.splice(from, 1);
        arr.splice(to, 0, moved);
        return { ...n, subnotes: arr };
      }) };
    }));
  }
  ...
  function addItem() {
    ...
    if (modal === "addChar") {
      ...
      const nc = { id: uid(), name: f.name, role: f.role || "", bio: f.bio || "", color: f.color || "#888", skinColor: "", eyeColor: "", hairColor: "", hairstyles: [], ethnicity: "", traits: [], noteSubs: [], outlineSubs: [], pinned: false };
      setStories(stories.map(s =>
        s.id === currentStoryId
      ...
      const members = (f.members || []).filter(Boolean);
      if (members.length < 2 || !f.description?.trim()) return;
      const nr = { id: uid(), members, description: f.description, pinned: false };
      setStories(stories.map(s => 
        s.id === currentStoryId 
  ...
  function getTabLabel(sec, itemId) {
    ...
    if (sec === "Relationships") {
      return relMembers(item).map(id => charMap[id]?.name || "?").join(" ↔ ");
    }
  ...
  function renderReadOnly(pos) {
    ...
    const PROSE = { fontFamily: "'Fredoka', sans-serif", fontSize: 15, color: "var(--c-c9b99a)", lineHeight: 1.8, whiteSpace: "pre-wrap" };
    const SCROLL = { flex: 1, overflowY: "auto", padding: "28px 36px", boxSizing: "border-box" };

    if (sec === "Home") return (
      <div style={SCROLL}>
        <div style={{ fontFamily: "'Bangers', cursive", fontSize: 36, color: "var(--c-ff1d8e)", textShadow: "3px 3px 0 var(--c-3a0a2e)", marginBottom: 20 }}>{currentStory.title}</div>
      ...
      return (
        <div style={SCROLL}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18 }}>
            <div style={{ width: 48, height: 48, borderRadius: "50%", background: ch.color, flexShrink: 0, border: "3px solid var(--c-2a2a2a)" }} />
            <div style={{ fontFamily: "'Bangers', cursive", fontSize: 34, color: "var(--c-ff1d8e)", textShadow: "2px 2px 0 var(--c-3a0a2e)" }}>{ch.name}</div>
          </div>
          ...
          {ch.traits?.length > 0 && <div style={{ marginTop: 14, display: "flex", flexWrap: "wrap", gap: 6 }}>
            {ch.traits.map(t => <span key={t} style={{ background: "var(--c-1a0828)", border: "2px solid var(--c-ff1d8e)", color: "var(--c-ff1d8e)", borderRadius: 20, padding: "2px 10px", fontSize: 12, fontWeight: 700 }}>{t}</span>)}
          </div>}
        </div>
      );
    }
      ...
      return (
        <div style={SCROLL}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
            <div style={{ fontFamily: "'Bangers', cursive", fontSize: 34, color: "var(--c-ff1d8e)", flex: 1, textShadow: "2px 2px 0 var(--c-3a0a2e)" }}>{ch.title}</div>
    ...
    if ((sec === "Notes" || sec === "Outline") && itemId) {
      ...
      return (
        <div style={SCROLL}>
          <div style={{ fontFamily: "'Bangers', cursive", fontSize: 30, color: "var(--c-ff1d8e)", marginBottom: 18, textShadow: "2px 2px 0 var(--c-3a0a2e)" }}>{note.title}</div>
          {(note.subnotes || []).map(sn => (
            <div key={sn.id} style={{ marginBottom: 16, background: "var(--c-141414)", border: "1px solid var(--c-2a2a2a)", borderRadius: 8, padding: "12px 16px" }}>
              <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: "var(--c-7dd3fc)", marginBottom: 6, fontSize: 14 }}>{sn.title}</div>
    ...
    if (sec === "Relationships" && itemId) {
      ...
      const mem = (rel.members && rel.members.length ? rel.members : [rel.charA, rel.charB].filter(Boolean)).map(id => currentStory.characters.find(c => c.id === id)).filter(Boolean);
      return (
        <div style={SCROLL}>
          <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 10, marginBottom: 18 }}>
            {mem.map((cm, i) => (
              <span key={cm.id} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {i > 0 && <span style={{ color: "#555", fontSize: 20, marginRight: 4 }}>↔</span>}
                <div style={{ width: 10, height: 10, borderRadius: "50%", background: cm.color || "#888" }} />
                <span style={{ fontFamily: "'Fredoka'", fontWeight: 700, color: "var(--c-7dd3fc)", fontSize: 18 }}>{cm.name || "?"}</span>
              </span>
            ))}
          </div>
    ...
    return (
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontFamily: "'Bangers', cursive", fontSize: 28, color: "var(--c-2a2a2a)", letterSpacing: "0.05em" }}>{sec}</div>
      </div>
    );
  }
  ...
  // Drag-reorder story folders (drop fromId before toId)
  ...
  function applyPDFImport(asNewStory) {
    ...
    const relationships = active
      ...
      .map(it => {
        const a = charByName[it.nameA?.toUpperCase()];
        const b = charByName[it.nameB?.toUpperCase()];
        if (!a || !b || a === b) return null;
        return { id: it.id, members: [a, b], description: it.content || "" };
      })
      .filter(Boolean);
  ...
  // ── per-character sub-notes (embedded Notes / Outline; field = noteSubs|outlineSubs) ──
  function mutateChar(charId, fn) {
    setStories(stories.map(s => s.id === currentStoryId ? { ...s, characters: s.characters.map(c => c.id === charId ? fn(c) : c) } : s));
  }
  function addCharSub(charId, field) {
    const sn = { id: uid(), title: field === "outlineSubs" ? "New Entry" : "New Note", content: "" };
    mutateChar(charId, c => ({ ...c, [field]: [...(c[field] || []), sn] }));
    setSelectedSubNotes(prev => ({ ...prev, [charId + ":" + field]: sn.id }));
  }
  function updateCharSub(charId, field, subId, key, value) {
    mutateChar(charId, c => ({ ...c, [field]: (c[field] || []).map(sn => sn.id === subId ? { ...sn, [key]: value } : sn) }));
  }
  function deleteCharSub(charId, field, subId) {
    mutateChar(charId, c => ({ ...c, [field]: (c[field] || []).filter(sn => sn.id !== subId) }));
    setSelectedSubNotes(prev => ({ ...prev, [charId + ":" + field]: null }));
  }
  function reorderCharSub(charId, field, fromId, toId) {
    if (!fromId || !toId || fromId === toId) return;
    mutateChar(charId, c => {
      const arr = [...(c[field] || [])];
  ...
  function renderDetail(section, selected) {
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "28px 40px 24px", boxSizing: "border-box" }}>
          <EditableText
            ...
            display={disguised ? fakeSubject(currentStory.title) : undefined}
            style={{ fontFamily: "'Bangers', cursive", fontSize: 42, letterSpacing: "0.03em", color: "var(--c-ff1d8e)", marginBottom: 20, textShadow: "3px 3px 0 var(--c-3a0a2e)", flexShrink: 0 }}
    ...
    if (section === "Characters") {
      ...
      // Embedded per-character notes/outline — tabbed sub-notes with the rich editor
      const renderCharSubs = (field) => {
        const subs = c[field] || [];
        const skey = c.id + ":" + field;
        const selId = selectedSubNotes[skey] || null;
        const cur = subs.find(sn => sn.id === selId);
        const noun = field === "outlineSubs" ? "entry" : "note";
        return (
          <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
            <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 8, marginBottom: 10, borderBottom: "2px dashed var(--c-2a2a2a)", flexShrink: 0 }}>
              {subs.map(sn => {
                const active = selId === sn.id;
                const isOver = dragOverSubId === sn.id && draggingSubId !== sn.id;
                return (
                  <button key={sn.id}
                    draggable
                    onDragStart={e => { e.stopPropagation(); setDraggingSubId(sn.id); }}
                    onDragOver={e => { e.preventDefault(); e.stopPropagation(); if (draggingSubId) setDragOverSubId(sn.id); }}
                    onDrop={e => { e.preventDefault(); e.stopPropagation(); reorderCharSub(c.id, field, draggingSubId, sn.id); setDraggingSubId(null); setDragOverSubId(null); }}
                    onDragEnd={() => { setDraggingSubId(null); setDragOverSubId(null); }}
                    onClick={() => setSelectedSubNotes(prev => ({ ...prev, [skey]: sn.id }))}
                    title="Drag to reorder"
                    style={{ background: active ? "var(--c-ff1d8e)" : "var(--c-1a1a1a)", border: `2px solid ${active ? "var(--c-ff1d8e)" : "var(--c-2a2a2a)"}`, borderLeft: isOver ? "3px solid var(--c-7dd3fc)" : (active ? "2px solid var(--c-ff1d8e)" : "2px solid var(--c-2a2a2a)"), color: active ? "var(--c-0d0d0d)" : "var(--c-b0a090)", padding: "6px 16px", borderRadius: 8, cursor: "grab", fontSize: 13, fontWeight: 600, fontFamily: "'Fredoka', sans-serif", whiteSpace: "nowrap", flexShrink: 0, opacity: draggingSubId === sn.id ? 0.4 : 1 }}>
                    {disguised ? fakeSubject(sn.title) : (sn.title || "Untitled")}
                  </button>
                );
              })}
              <button onClick={() => addCharSub(c.id, field)} style={{ background: "none", border: "2px dashed #3a3a3a", color: "#666", padding: "6px 14px", borderRadius: 8, cursor: "pointer", fontSize: 18, lineHeight: 1, flexShrink: 0 }}>+</button>
            </div>
            {cur ? (
              <>
                <EditableText val={cur.title} display={disguised ? fakeSubject(cur.title) : undefined} style={{ fontSize: 16, fontWeight: 700, color: "var(--c-c9b99a)", lineHeight: 1.3, marginBottom: 8, flexShrink: 0 }} onEdit={v => updateCharSub(c.id, field, cur.id, "title", v)} />
                <ChapterEditor key={cur.id} content={cur.content || ""}
                  requestLink={cb => { setLinkPicker({ onInsert: cb }); setLinkPickerQuery(""); setLinkPickerSection("Characters"); }}
                  onNavigate={(sec, id) => { setSection(sec); setSelected(id); }}
                  onSave={v => updateCharSub(c.id, field, cur.id, "content", v)} />
                <div style={{ display: "flex", gap: 10, flexShrink: 0, marginTop: 16 }}>
                  <button style={styles.deleteBtn} onClick={() => setItemToDelete({ type: "char-sub", charId: c.id, field, id: cur.id, name: cur.title || "Untitled" })}>Delete {noun}</button>
                </div>
              </>
            ) : (
              <div style={{ color: "#555", fontStyle: "italic", fontSize: 14, flex: 1 }}>{subs.length === 0 ? `No ${noun}s yet — hit + to add one.` : `Select a ${noun} above.`}</div>
            )}
          </div>
        );
      };
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%", boxSizing: "border-box" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
            <div style={{ width: 52, height: 52, borderRadius: "50%", background: c.color, flexShrink: 0, border: "3px solid var(--c-2a2a2a)", boxShadow: "3px 3px 0 var(--c-0d0d0d)" }} />
            <EditableText val={c.name} display={disguised ? fakeSubject(c.name) : undefined} style={styles.detailTitle} onEdit={v => updateField("characters", c.id, "name", v)} />
          </div>

          {/* Tab bar */}
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[["detail", "Detail"], ["notes", "Notes"], ["outline", "Outline"]].map(([tab, label]) => (
              <button
                key={tab}
                onClick={() => setCharTabs(prev => ({ ...prev, [c.id]: tab }))}
                style={{
                  background: charTab === tab ? "var(--c-ff1d8e)" : "none",
                  border: `2px solid ${charTab === tab ? "var(--c-ff1d8e)" : "var(--c-2a2a2a)"}`,
                  color: charTab === tab ? "var(--c-0d0d0d)" : "#777",
                  padding: "7px 20px",
                  borderRadius: 8,
                  cursor: "pointer",
                  fontSize: 13,
                  ...
                  boxShadow: charTab === tab ? "2px 2px 0 var(--c-3a0a2e)" : "none",
                }}
              >
                {label}
              </button>
          ...
          {/* body: Notes/Outline use the embedded editor; Detail scrolls */}
          {(charTab === "notes" || charTab === "outline") ? (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, padding: "10px 40px 20px", boxSizing: "border-box" }}>
              {renderCharSubs(charTab === "notes" ? "noteSubs" : "outlineSubs")}
            </div>
          ) : (
          ...
          <>
            ...
            <div style={{ borderTop: "2px dashed var(--c-2a2a2a)", paddingTop: 20, marginBottom: 4 }}>
              <div style={{ fontFamily: "'Bangers', cursive", fontSize: 20, letterSpacing: "0.05em", color: "var(--c-7dd3fc)", marginBottom: 16, textShadow: "2px 2px 0 var(--c-0a203a)" }}>Appearance</div>
            ...
            <div style={{ borderTop: "2px dashed var(--c-2a2a2a)", paddingTop: 20, marginBottom: 4 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                <div style={{ fontFamily: "'Bangers', cursive", fontSize: 20, letterSpacing: "0.05em", color: "var(--c-7dd3fc)", textShadow: "2px 2px 0 var(--c-0a203a)" }}>Character Traits</div>
              ...
              {(c.traits || []).length === 0
                ...
                : <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
                    {c.traits.map(trait => (
                      <div key={trait} style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--c-1a0828)", border: "2px solid var(--c-ff1d8e)", borderRadius: 20, padding: "4px 8px 4px 12px", boxShadow: "2px 2px 0 var(--c-3a0a2e)" }}>
                        <span style={{ fontSize: 12, color: "var(--c-ff1d8e)", fontWeight: 700, fontFamily: "'Fredoka', sans-serif" }}>{trait}</span>
                        <button onClick={() => updateField("characters", c.id, "traits", c.traits.filter(t => t !== trait))} style={{ background: "none", border: "none", color: "var(--c-c050a0)", cursor: "pointer", fontSize: 16, lineHeight: 1, padding: "0 2px", fontWeight: 700 }}>×</button>
                      </div>
                    ))}
                  </div>
              }
            ...
            {rels.length > 0 && <div style={{ borderTop: "2px dashed var(--c-2a2a2a)", paddingTop: 20, marginBottom: 4 }}>
              <div style={{ fontFamily: "'Bangers', cursive", fontSize: 20, letterSpacing: "0.05em", color: "var(--c-7dd3fc)", marginBottom: 12, textShadow: "2px 2px 0 var(--c-0a203a)" }}>Relationships</div>
              {rels.map(r => {
                const others = relMembers(r).filter(m => m !== c.id).map(m => charMap[m]).filter(Boolean);
                return (
                  <div key={r.id} style={styles.relCard}>
                    <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 10, marginBottom: 8 }}>
                      {others.length === 0 && <span style={{ color: "#555", fontSize: 14 }}>—</span>}
                      {others.map(o => (
                        <span key={o.id} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                          <div style={{ width: 10, height: 10, borderRadius: "50%", background: o.color || "#888" }} />
                          <span style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 600, fontSize: 15, color: "var(--c-7dd3fc)" }}>{o.name || "Unknown"}</span>
                        </span>
                      ))}
                    </div>
            ...
            <div style={{ borderTop: "2px dashed var(--c-2a2a2a)", paddingTop: 20, marginBottom: 4 }}>
              <div style={{ fontFamily: "'Bangers', cursive", fontSize: 20, letterSpacing: "0.05em", color: "var(--c-7dd3fc)", marginBottom: 12, textShadow: "2px 2px 0 var(--c-0a203a)" }}>Appears In</div>
              {mentions.length === 0
                ...
                : <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
                    {mentions.map(m => (
                      <button key={`${m.sec}-${m.id}`} onClick={() => { setSection(m.sec); setSelected(m.id); }}
                        style={{ background: "var(--c-0d1428)", border: "2px solid var(--c-1a2840)", color: "var(--c-7dd3fc)", borderRadius: 8, padding: "4px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Fredoka', sans-serif" }}>
    ...
    if (section === "Relationships") {
      ...
      if (!r) return null;
      const members = relMembers(r);
      const setMembers = next => updateField("relationships", r.id, "members", next);
      const available = currentStory.characters.filter(c => !members.includes(c.id));
      return (
        <div style={styles.detail}>
          <Label>Characters</Label>
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, marginBottom: 24 }}>
            {members.map(mid => (
              <span key={mid} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "var(--c-0a1428)", border: "2px solid var(--c-1a2840)", borderRadius: 20, padding: "4px 8px 4px 12px", boxShadow: "2px 2px 0 var(--c-0d0d0d)" }}>
                <span style={{ width: 10, height: 10, borderRadius: "50%", background: charMap[mid]?.color || "#888" }} />
                <span title="Open character" onClick={() => { setSection("Characters"); setSelected(mid); }} style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 600, color: "var(--c-7dd3fc)", fontSize: 15, cursor: "pointer" }}>{charMap[mid]?.name || "?"}</span>
                {members.length > 2 && <button title="Remove from relationship" onClick={() => setMembers(members.filter(m => m !== mid))} style={{ background: "none", border: "none", color: "var(--c-c050a0)", cursor: "pointer", fontSize: 16, lineHeight: 1, padding: "0 2px" }}>×</button>}
              </span>
            ))}
            {available.length > 0 && (
              <select value="" onChange={e => { if (e.target.value) setMembers([...members, e.target.value]); }}
                style={{ ...styles.select, width: "auto", padding: "6px 10px", fontSize: 13 }}>
          ...
          <button style={styles.deleteBtn} onClick={() => { setItemToDelete({ type: "relationship", id: r.id, name: relLabel(r) }); }}>Delete Relationship</button>
        </div>
      );
    }
    ...
    function renderTimelineStrip(list, { renderDot, renderBody, onOpen, addLabel, addModal, renderFlag }) {
      ...
      const card = (item, i) => {
        ...
        return (
          <div style={{ width: 190, flexShrink: 0, position: "relative", opacity: draggingId === item.id ? 0.4 : 1 }}
            ...
            {isOver && <div style={{ position: "absolute", left: -14, top: 14, bottom: 0, width: 2, background: "var(--c-ff1d8e)", borderRadius: 1 }} />}
            <div style={{ height: 16, lineHeight: "16px", textAlign: "center", fontFamily: "'Bangers', cursive", fontSize: 14, letterSpacing: "0.08em", color: "var(--c-c050a0)", textShadow: "1px 1px 0 var(--c-3a0a2e)", whiteSpace: "nowrap" }}>
              {flag || ""}
            </div>
            <div style={{ width: 12, height: 12, borderRadius: "50%", background: renderDot(item), border: "2px solid var(--c-111)", margin: "4px auto 12px", position: "relative" }} />
            <div onClick={() => onOpen(item)}
              style={{ background: "var(--c-141414)", border: "2px solid var(--c-2a2a2a)", borderRadius: 12, padding: 16, cursor: "pointer", transition: "border-color 0.15s" }}
              onMouseEnter={e => e.currentTarget.style.borderColor = "var(--c-ff1d8e)"}
              onMouseLeave={e => e.currentTarget.style.borderColor = "var(--c-2a2a2a)"}>
              {renderBody(item, i)}
            </div>
          </div>
        );
      ...
      const addCard = (
        <div style={{ width: 190, flexShrink: 0 }}>
          <div style={{ height: 16 }} />
          <div style={{ width: 12, height: 12, borderRadius: "50%", border: "2px dashed #3a3a3a", margin: "4px auto 12px", boxSizing: "border-box", background: "var(--c-111)" }} />
      ...
      return renderTimelineStrip(currentStory.chapters || [], {
        ...
        renderBody: (ch, i) => {
          ...
          return <>
            ...
            <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: "var(--c-c9b99a)", fontSize: 14, marginBottom: 10, lineHeight: 1.3 }}>{ch.title || "Untitled"}</div>
    ...
    if (section === "OutlineTimeline") {
      return renderTimelineStrip(currentStory.outlines || [], {
        ...
        renderBody: (g, i) => <>
          ...
          <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: "var(--c-c9b99a)", fontSize: 14, marginBottom: 10, lineHeight: 1.3 }}>{g.title || "Untitled"}</div>
      ...
      return (
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", overflow: "auto", padding: 24 }}>
          <svg width={W} height={H} style={{ overflow: "visible" }}>
            {rels.map(r => {
              const pts = relMembers(r).map(id => posMap[id]).filter(Boolean);
              if (pts.length < 2) return null;
              const lines = [];
              for (let i = 0; i < pts.length; i++)
                for (let j = i + 1; j < pts.length; j++)
                  lines.push(<line key={`${i}-${j}`} x1={pts[i].x} y1={pts[i].y} x2={pts[j].x} y2={pts[j].y} stroke="var(--c-2a2a2a)" strokeWidth={2} />);
            ...
            {positions.map(({ c, x, y }) => (
              <g key={c.id} style={{ cursor: "pointer" }} onClick={() => { setSection("Characters"); setSelected(c.id); setCharTab("detail"); openNewTab("Characters", c.id); }}>
                <circle cx={x} cy={y} r={28} fill={c.color || "#888"} stroke="var(--c-2a2a2a)" strokeWidth={3} />
                <text x={x} y={y + 44} textAnchor="middle" fontSize={12} fill="var(--c-c9b99a)" fontFamily="Fredoka" fontWeight={600}>{c.name}</text>
              </g>
            ))}
          </svg>
        </div>
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "28px 40px 24px", boxSizing: "border-box" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0, marginBottom: 16 }}>
            <EditableText val={item.title} display={disguised ? fakeSubject(item.title) : undefined} style={{ ...styles.detailTitle, marginBottom: 0, flex: 1 }} onEdit={v => updateField("chapters", item.id, "title", v)} />
            <button onClick={() => cycleChapterAct(item.id)} title="Cycle act"
              style={{ ...CHIP, background: item.act ? "var(--c-28102a)" : "none", border: `2px solid ${item.act ? "var(--c-c050a0)" : "var(--c-2a2a2a)"}`, color: item.act ? "var(--c-c050a0)" : "#555" }}>
              {item.act || "No Act"}
            </button>
            ...
            <button onClick={() => setShowSnapshots(v => !v)} title="Snapshots"
              style={{ ...CHIP, background: showSnapshots ? "var(--c-0a203a)" : "none", border: "2px solid var(--c-7dd3fc)", color: "var(--c-7dd3fc)" }}>
              📸 {snapshots.length}
            </button>
            <button onClick={() => setFocusChapterId(item.id)} title="Focus mode (Esc to exit)"
              style={{ ...CHIP, background: "none", border: "2px solid var(--c-2a2a2a)", color: "#888" }}>
              ⛶ Focus
            </button>
          </div>

          {showSnapshots && (
            <div style={{ flexShrink: 0, marginBottom: 16, background: "var(--c-0d0d0d)", border: "1px solid var(--c-2a2a2a)", borderRadius: 10, padding: "10px 14px", maxHeight: 200, overflowY: "auto" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                ...
                <button style={{ background: "var(--c-0a203a)", border: "2px solid var(--c-7dd3fc)", color: "var(--c-7dd3fc)", padding: "3px 12px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: "'Fredoka', sans-serif" }}
              ...
              {[...snapshots].reverse().map(sn => {
                ...
                return (
                  <div key={sn.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 0", borderBottom: "1px solid var(--c-1a1a1a)" }}>
                    <span style={{ fontSize: 12, color: "var(--c-c9b99a)", flexShrink: 0 }}>{new Date(sn.at).toLocaleString()}</span>
                    {sn.label && <span style={{ fontSize: 10, color: "var(--c-c050a0)", border: "1px solid var(--c-c050a0)", borderRadius: 10, padding: "1px 8px", flexShrink: 0 }}>{sn.label}</span>}
                    <span style={{ fontSize: 11, color: "#555", flex: 1 }}>{wc} words</span>
                    <button style={{ background: "none", border: "1px solid var(--c-7dd3fc)", color: "var(--c-7dd3fc)", padding: "2px 10px", borderRadius: 4, cursor: "pointer", fontSize: 11, fontFamily: "'Fredoka', sans-serif", flexShrink: 0 }}
    ...
    if (section === "Notes" || section === "Outline") {
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "16px 40px 20px", boxSizing: "border-box" }}>

          {/* header row */}
          <div style={{ flexShrink: 0, marginBottom: 8 }}>
            <EditableText val={item.title} display={disguised ? fakeSubject(item.title) : undefined} style={styles.detailTitle} onEdit={v => updateField(collKey, item.id, "title", v)} />
            {linkedChar && (
              <button style={{ background: "none", border: "2px solid var(--c-ff1d8e)", color: "var(--c-ff1d8e)", padding: "4px 14px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: "'Fredoka', sans-serif", marginTop: 8 }} onClick={() => { setSection("Characters"); setSelected(linkedChar.id); setCharTab("detail"); }}>
                ← {linkedChar.name}
              </button>
            )}
          </div>
          ...
          {/* subnote tabs */}
          <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 8, marginBottom: 10, borderBottom: "2px dashed var(--c-2a2a2a)", scrollbarWidth: "thin", scrollbarColor: "#333 transparent", flexShrink: 0 }}>
            {subnotes.map(sn => {
              const active = selectedSubNoteId === sn.id;
              const isOver = dragOverSubId === sn.id && draggingSubId !== sn.id;
              return (
                <button key={sn.id}
                  draggable
                  onDragStart={e => { e.stopPropagation(); setDraggingSubId(sn.id); }}
                  onDragOver={e => { e.preventDefault(); e.stopPropagation(); if (draggingSubId) setDragOverSubId(sn.id); }}
                  onDrop={e => { e.preventDefault(); e.stopPropagation(); reorderSubNote(item.id, draggingSubId, sn.id, collKey); setDraggingSubId(null); setDragOverSubId(null); }}
                  onDragEnd={() => { setDraggingSubId(null); setDragOverSubId(null); }}
                  onClick={() => setSelectedSubNotes(prev => ({ ...prev, [item.id]: sn.id }))}
                  title="Drag to reorder"
                  style={{ background: active ? "var(--c-ff1d8e)" : "var(--c-1a1a1a)", border: `2px solid ${active ? "var(--c-ff1d8e)" : "var(--c-2a2a2a)"}`, borderLeft: isOver ? "3px solid var(--c-7dd3fc)" : (active ? "2px solid var(--c-ff1d8e)" : "2px solid var(--c-2a2a2a)"), color: active ? "var(--c-0d0d0d)" : "var(--c-b0a090)", padding: "6px 16px", borderRadius: 8, cursor: "grab", fontSize: 13, fontWeight: 600, fontFamily: "'Fredoka', sans-serif", whiteSpace: "nowrap", flexShrink: 0, opacity: draggingSubId === sn.id ? 0.4 : 1, boxShadow: active ? "2px 2px 0 var(--c-3a0a2e)" : "none" }}>
                  {sn.title || "Untitled"}
                </button>
              );
            })}
          ...
          {currentSub ? (
            <>
              {/* subnote title */}
              <EditableText val={currentSub.title} display={disguised ? fakeSubject(currentSub.title) : undefined} style={{ fontSize: 16, fontWeight: 700, color: "var(--c-c9b99a)", lineHeight: 1.3, marginBottom: 8, flexShrink: 0 }} onEdit={v => updateSubNote(item.id, currentSub.id, "title", v, collKey)} />
    ...
    return (
      <div style={styles.overlay} onClick={e => { if (e.target === e.currentTarget) { setModal(null); setNewForm({}); } }}>
        <div style={styles.modalBox} onKeyDown={e => { if (e.key === "Enter" && e.target.tagName === "INPUT") addItem(); }}>
          <h3 style={{ fontFamily: "'Bangers', cursive", fontSize: 28, letterSpacing: "0.04em", color: "var(--c-ff1d8e)", marginBottom: 20, borderBottom: "2px dashed var(--c-3a0a2e)", paddingBottom: 12 }}>
          ...
          {modal === "addRel" && <>
            <div style={{ marginBottom: 12 }}>
              <label style={styles.formLabel}>Characters * (add at least 2)</label>
              {(newForm.members || []).length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                  {(newForm.members || []).map(mid => (
                    <span key={mid} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "var(--c-0a1428)", border: "2px solid var(--c-1a2840)", color: "var(--c-7dd3fc)", borderRadius: 20, padding: "3px 8px 3px 12px", fontSize: 13, fontWeight: 600 }}>
                      <span style={{ width: 8, height: 8, borderRadius: "50%", background: charMap[mid]?.color || "#888" }} />
                      {charMap[mid]?.name || "?"}
                      <button onClick={() => setNewForm(f => ({ ...f, members: (f.members || []).filter(x => x !== mid) }))} style={{ background: "none", border: "none", color: "var(--c-7dd3fc)", cursor: "pointer", fontSize: 15, lineHeight: 1, padding: 0 }}>×</button>
                    </span>
                  ))}
                </div>
              )}
              <select style={styles.select} value="" onChange={e => { const v = e.target.value; if (v) setNewForm(f => ({ ...f, members: [...(f.members || []), v] })); }}>
                <option value="">+ add character…</option>
                {currentStory.characters.filter(c => !(newForm.members || []).includes(c.id)).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
            ...
            <button style={{ ...styles.addBtn, background: "var(--c-2a2a2a)", color: "#888" }} onClick={() => { setModal(null); setNewForm({}); }}>Cancel</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={styles.overlay} onClick={e => { if (e.target === e.currentTarget) { setShowStoryModal(false); setNewForm({}); } }}>
        <div style={styles.modalBox} onKeyDown={e => { if (e.key === "Enter" && e.target.tagName === "INPUT") createStory(); }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
            ...
            <button style={{ ...styles.addBtn, background: "var(--c-2a2a2a)", color: "#888" }} onClick={() => { setShowStoryModal(false); setNewForm({}); }}>Cancel</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div style={styles.overlay} onClick={() => { setShowImportModal(false); setImportData(null); }}>
        <div style={styles.modalBox} onClick={e => e.stopPropagation()}>
          <h3 style={{ fontFamily: "'Bangers', cursive", fontSize: 28, letterSpacing: "0.04em", color: "var(--c-ff1d8e)", marginBottom: 16, borderBottom: "2px dashed var(--c-3a0a2e)", paddingBottom: 12 }}>Import Backup</h3>
          <div style={{ fontSize: 14, color: "var(--c-c9b99a)", marginBottom: 12 }}>
            Found <strong style={{ color: "var(--c-7dd3fc)" }}>{importData.length}</strong> stor{importData.length === 1 ? "y" : "ies"} in this backup file.
          </div>
          <div style={{ background: "var(--c-0d0d0d)", border: "1px solid var(--c-2a2a2a)", borderRadius: 8, padding: "12px 14px", marginBottom: 20 }}>
            {importData.map(s => (
              <div key={s.id} style={{ fontSize: 13, color: "var(--c-b0a090)", marginBottom: 4 }}>• {s.title || "Untitled"}</div>
            ))}
          </div>
          <div style={{ fontSize: 13, color: "#666", marginBottom: 24, lineHeight: 1.6 }}>
            <span style={{ color: "var(--c-7dd3fc)", fontWeight: 700 }}>Merge</span> — adds/updates stories from the backup, keeps anything not in the file.<br />
            <span style={{ color: "var(--c-ff1d8e)", fontWeight: 700 }}>Replace All</span> — wipes your current data and loads only the backup.
          </div>
    ...
    return (
      <div style={{ ...styles.overlay, zIndex: 9999 }}>
        <div style={{ ...styles.modalBox, textAlign: "center", maxWidth: 360 }}>
          <h3 style={{ fontFamily: "'Bangers', cursive", fontSize: 30, letterSpacing: "0.06em", color: "var(--c-ff1d8e)", marginBottom: 12, borderBottom: "2px dashed var(--c-3a0a2e)", paddingBottom: 12 }}>
            Before You Go
          </h3>
          <p style={{ fontSize: 14, color: "var(--c-c9b99a)", marginBottom: 24, lineHeight: 1.6 }}>
            Save a backup before closing?
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <button
              style={{ background: "var(--c-7dd3fc)", border: "2px solid var(--c-3a6a8a)", color: "var(--c-0d0d0d)", padding: "10px 16px", borderRadius: 6, cursor: "pointer", fontSize: 14, fontWeight: 700, fontFamily: "'Fredoka', sans-serif" }}
              onClick={async () => {
  ...
  if (!currentStoryId) {
    return (
      <div style={disguised ? { ...styles.root, paddingTop: 94, boxSizing: "border-box" } : styles.root}>
      {disguised && <OutlookRibbon onExit={() => setDisguised(false)} dark={disguiseDark} onToggleDark={() => setDisguiseDark(d => !d)} />}
        ...
        <div style={{ ...styles.sidebar, width: sidebarCollapsed ? 40 : sidebarWidth, position: "relative", overflow: "hidden", transition: "width 0.15s" }}>
          ...
          {sidebarCollapsed ? (
            <button onClick={() => { setSidebarCollapsed(false); setSidebarWidth(lastSidebarWidth.current); }} style={{ width: 40, height: 40, background: "none", border: "none", color: "var(--c-ff1d8e)", cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
          ) : (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 12px 0 20px" }}>
              <div style={styles.brand}>{disguised ? "Outlook" : "QWOSID"}</div>
          ...
          <div style={{ marginBottom: 16, padding: "0 20px" }}>
            ...
            {(() => {
              ...
              const renderStoryRow = (s, indent = false) => {
                ...
                return (
                  <div key={s.id}
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", paddingLeft: indent ? 28 : 10, borderRadius: 6, marginBottom: 2, cursor: "grab", opacity: isDragging ? 0.4 : 1, background: isOver ? "var(--c-2a1428)" : "transparent", borderTop: isOver ? "2px solid var(--c-ff1d8e)" : "2px solid transparent" }}
                  >
                    {isEditingStories && (
                    ...
                    {renamingStoryId === s.id ? (
                      ...
                      <span title={s.title} onDoubleClick={e => { e.stopPropagation(); setRenamingStoryId(s.id); }}
                        style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 600, color: "var(--c-b0a090)", fontSize: 13, flex: 1, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", cursor: isEditingStories ? "text" : "pointer" }}>
                        {s.title}
                      </span>
                    )}
              ...
              return (
                <div style={{ maxHeight: 360, overflowY: "auto", paddingRight: 4 }}
                  onDragOver={e => { if (draggingId || draggingFolderId) { e.preventDefault(); setDragOverId("ungroup"); } }}
                  onDrop={e => { e.preventDefault(); if (draggingId) { setStoryFolder(draggingId, null); } setDraggingId(null); setDraggingFolderId(null); setDragOverId(null); }}
                >
                    ...
                    return (
                      <div key={folder.id} style={{ marginTop: 4 }}>
                        <div
                          ...
                          onDragStart={e => { e.stopPropagation(); setDraggingFolderId(folder.id); setDraggingId(null); }}
                          onDragOver={e => { if (draggingId || draggingFolderId) { e.preventDefault(); e.stopPropagation(); setDragOverId("f-" + folder.id); } }}
                          onDrop={e => { e.preventDefault(); e.stopPropagation(); if (draggingFolderId) { reorderFolders(draggingFolderId, folder.id); } else if (draggingId) { setStoryFolder(draggingId, folder.id); } setDraggingId(null); setDraggingFolderId(null); setDragOverId(null); }}
                          onDragEnd={() => { setDraggingFolderId(null); setDragOverId(null); }}
                          style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 10px", borderRadius: 6, background: over ? "var(--c-2a1428)" : "var(--c-161616)", cursor: "grab", borderTop: over ? "2px solid var(--c-ff1d8e)" : "2px solid transparent", marginBottom: 2, opacity: draggingFolderId === folder.id ? 0.4 : 1 }}
                        >
                          ...
                          {renamingFolderId === folder.id ? (
                            ...
                            <span onDoubleClick={e => { e.stopPropagation(); setRenamingFolderId(folder.id); }}
                              style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: "var(--c-c9b99a)", fontSize: 13, flex: 1, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
            ...
            <div style={{ marginTop: 12 }}>
              <div style={{ display: "flex", gap: 6 }}>
                <button style={{ flex: 1, background: "none", border: "2px solid var(--c-ff1d8e)", color: "var(--c-ff1d8e)", padding: "7px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: "'Fredoka', sans-serif" }} onClick={() => setShowStoryModal(true)}>+ Story</button>
                <button style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, background: "none", border: "2px solid var(--c-2a2a2a)", color: "#888", padding: "7px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: "'Fredoka', sans-serif" }} onClick={createFolder}><FolderIcon /> Folder</button>
              </div>
              <button style={{ background: isEditingStories ? "var(--c-ff1d8e)" : "none", border: `1px solid ${isEditingStories ? "var(--c-ff1d8e)" : "#333"}`, color: isEditingStories ? "var(--c-0d0d0d)" : "#888", padding: "6px 8px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: "'Fredoka', sans-serif", width: "100%", marginTop: 8 }} onClick={() => { setIsEditingStories(!isEditingStories); setSelectedStories(new Set()); }}>
                {isEditingStories ? "Done" : "Select to Delete"}
              </button>
              {isEditingStories && selectedStories.size > 0 && (
                <button style={{ background: "var(--c-ff1d8e)", border: "2px solid var(--c-3a0a2e)", color: "var(--c-0d0d0d)", padding: "8px 16px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: "'Fredoka', sans-serif", width: "100%", marginTop: 6 }} onClick={() => setShowDeleteConfirm(true)}>
                  Delete Selected ({selectedStories.size})
                </button>
              )}
            </div>
          </div>
          <div style={{ marginTop: "auto", padding: "16px 20px", borderTop: "2px dashed var(--c-2a2a2a)" }}>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 8 }}>Backup</div>
            <button style={{ background: "none", border: "2px solid var(--c-7dd3fc)", color: "var(--c-7dd3fc)", padding: "7px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, width: "100%", fontFamily: "'Fredoka', sans-serif", marginBottom: 6 }} onClick={exportBackup}>Download Backup</button>
            <button style={{ background: "none", border: "2px dashed var(--c-7dd3fc)", color: "var(--c-7dd3fc)", padding: "7px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, width: "100%", fontFamily: "'Fredoka', sans-serif" }} onClick={() => importInputRef.current?.click()}>Import Backup</button>
        ...
        {/* main content */}
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--c-111)" }}>
          <div style={{ textAlign: "center", color: "#888" }}>
            <div style={{ fontSize: 72, fontFamily: "'Bangers', cursive", letterSpacing: "0.04em", color: "var(--c-ff1d8e)", textShadow: "4px 4px 0 var(--c-3a0a2e), 8px 8px 0 rgba(0,0,0,0.4)" }}>{disguised ? "Outlook" : "QWOSID"}</div>
          </div>
        </div>
        ...
        {showDeleteConfirm && (
          <div style={styles.overlay} onClick={() => setShowDeleteConfirm(false)}>
            <div style={styles.modalBox} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ fontSize: 14, color: "var(--c-c9b99a)", marginBottom: 24 }}>
                Are you sure you want to delete the following story{selectedStories.size > 1 ? 's' : ''}?
              </div>
              <div style={{ background: "var(--c-161616)", border: "1px solid var(--c-252525)", borderRadius: 6, padding: "14px 16px", marginBottom: 24 }}>
                {Array.from(selectedStories).map(storyId => {
                  const story = stories.find(s => s.id === storyId);
                  return (
                    <div key={storyId} style={{ marginBottom: 8, fontSize: 14, color: "var(--c-b0a090)" }}>
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                ...
                <button style={{ background: "var(--c-ff1d8e)", border: "2px solid var(--c-3a0a2e)", color: "var(--c-0d0d0d)", padding: "8px 16px", borderRadius: 4, cursor: "pointer", fontSize: 12 }} onClick={() => {
  ...
  function ViewToggle({ views }) {
    return (
      <div style={{ display: "inline-flex", background: "var(--c-1a1a1a)", border: "1px solid var(--c-2a2a2a)", borderRadius: 14, padding: 2, gap: 2 }}>
        {views.map(v => {
          const active = section === v.sec;
          return (
            <button key={v.sec}
              onClick={() => { if (!active) { setSection(v.sec); setSelected(null); } }}
              style={{ background: active ? "var(--c-ff1d8e)" : "none", color: active ? "var(--c-0d0d0d)" : "#666", border: "none", borderRadius: 12, padding: "3px 12px", fontSize: 10, fontWeight: 700, cursor: active ? "default" : "pointer", fontFamily: "'Fredoka', sans-serif", letterSpacing: "0.04em" }}>
              {v.label}
            </button>
          );
        })}
  ...
  function renderStorageStatus() {
    return (
      <div style={{ margin: "16px 20px", paddingTop: 14, borderTop: "2px dashed var(--c-2a2a2a)" }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 6 }}>Storage</div>
        <div style={{ fontSize: 12, color: isSaving ? "var(--c-ff1d8e)" : saveStatus ? "#4caf50" : "#444", fontWeight: 600, minHeight: 18, transition: "color 0.3s" }}>
        ...
        <button title="Disguise as Outlook (Ctrl+Shift+O)" onClick={() => setDisguised(d => !d)}
          style={{ marginTop: 10, width: "100%", background: "none", border: "1px solid var(--c-2a2a2a)", color: "#666", padding: "5px 8px", borderRadius: 6, cursor: "pointer", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", fontFamily: "'Fredoka', sans-serif" }}>
          {disguised ? "Exit Work Mode" : "🕶 Work Mode"}
        </button>
        {disguised && (
          <button title="Toggle Outlook light/dark" onClick={() => setDisguiseDark(d => !d)}
            style={{ marginTop: 6, width: "100%", background: "none", border: "1px solid var(--c-2a2a2a)", color: "#666", padding: "5px 8px", borderRadius: 6, cursor: "pointer", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", fontFamily: "'Fredoka', sans-serif" }}>
            {disguiseDark ? "☀ Light Outlook" : "☾ Dark Outlook"}
          </button>
        )}
      </div>
    );
  ...
  if (!currentStory) {
    return (
      ...
      {disguised && <OutlookRibbon onExit={() => setDisguised(false)} dark={disguiseDark} onToggleDark={() => setDisguiseDark(d => !d)} />}
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--c-111)" }}>
          <div style={{ textAlign: "center", color: "#888" }}>
            <div style={{ fontSize: 48, fontFamily: "'Bangers', cursive", letterSpacing: "0.04em", color: "var(--c-ff1d8e)", textShadow: "3px 3px 0 var(--c-3a0a2e)" }}>{disguised ? "Outlook" : "QWOSID"}</div>
          </div>
        </div>
        {renderCloseModal()}
      </div>
  ...
  return (
    <div style={disguised ? { ...styles.root, paddingTop: 94, boxSizing: "border-box" } : styles.root}>
      ...
      <div style={{ ...styles.sidebar, width: sidebarCollapsed ? 40 : sidebarWidth, position: "relative", overflowX: "hidden", overflowY: "auto", transition: "width 0.15s" }}>
        ...
        {sidebarCollapsed ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <button title="Back to stories" onClick={() => setCurrentStoryId(null)}
              style={{ background: "none", border: "2px solid var(--c-ff1d8e)", color: "var(--c-ff1d8e)", width: 30, height: 30, borderRadius: "50%", cursor: "pointer", fontSize: 16, fontWeight: 700, fontFamily: "'Fredoka', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, lineHeight: 1 }}>
              ←
            </button>
            <span style={{ fontFamily: "'Bangers', cursive", color: "var(--c-7dd3fc)", fontSize: 20, letterSpacing: "0.04em", textShadow: "2px 2px 0 var(--c-3a0a2e)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{currentStory.title}</span>
          </div>
        </div>
          ...
          {[{ sec: "Search", icon: "🔍" }, { sec: "Trash", icon: "🗑" }].map(({ sec, icon }) => {
            ...
            return (
              <button key={sec} title={sec} onClick={() => { setSection(sec); setSelected(null); }}
                style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, background: active ? "var(--c-280820)" : "none", border: `2px solid ${active ? "var(--c-ff1d8e)" : "var(--c-2a2a2a)"}`, color: active ? "var(--c-ff1d8e)" : "#666", borderRadius: 8, padding: "8px 4px", cursor: "pointer", fontFamily: "'Fredoka', sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", transition: "color 0.15s, border-color 0.15s" }}>
        ...
        <div style={{ padding: "0 20px 20px" }}>
          <div>
            ...
            <button style={{ background: "none", border: "2px solid var(--c-ff1d8e)", color: "var(--c-ff1d8e)", padding: "7px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, width: "100%", fontFamily: "'Fredoka', sans-serif", marginBottom: 14 }} onClick={() => exportStoryAsPDF(currentStory)}>Export Story PDF</button>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 8 }}>Backup</div>
            <button style={{ background: "none", border: "2px solid var(--c-7dd3fc)", color: "var(--c-7dd3fc)", padding: "7px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, width: "100%", fontFamily: "'Fredoka', sans-serif", marginBottom: 6 }} onClick={exportBackup}>Download Backup</button>
            <button style={{ background: "none", border: "2px dashed var(--c-7dd3fc)", color: "var(--c-7dd3fc)", padding: "7px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, width: "100%", fontFamily: "'Fredoka', sans-serif" }} onClick={() => importInputRef.current?.click()}>Import Backup</button>
            <input ref={importInputRef} type="file" accept=".json" style={{ display: "none" }} onChange={handleImportFile} />
            ...
            <button style={{ background: "none", border: "2px dashed var(--c-c050a0)", color: "var(--c-c050a0)", padding: "7px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, width: "100%", fontFamily: "'Fredoka', sans-serif" }} onClick={() => pdfImportRef.current?.click()}>{pdfParsing ? "Parsing…" : "Import PDF"}</button>
      ...
      <div style={{ ...styles.list, width: listWidth, position: "relative" }}>
          ...
          {FOLDER_SECTIONS.includes(section) && (
            <div style={{ display: "flex", marginTop: 8, alignItems: "center" }}>
              <button title="Add a folder" onClick={() => addItemFolder(section, null)}
                style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "1px solid var(--c-2a2a2a)", color: "#888", padding: "3px 10px", borderRadius: 12, cursor: "pointer", fontSize: 10, fontWeight: 700, fontFamily: "'Fredoka', sans-serif", letterSpacing: "0.04em" }}>
        ...
        {section === "Home" ? (
          <div style={{ overflowY: "auto", flex: 1 }}>
            {[
              { label: "Characters", items: currentStory.characters, color: "var(--c-ff1d8e)", dot: true },
              { label: "Chapters",   items: currentStory.chapters,   color: "var(--c-7dd3fc)", dot: false },
              { label: "Notes",      items: currentStory.notes,      color: "var(--c-c9b99a)", dot: false },
            ].map(({ label, items: grpItems, color, dot }) => grpItems.length === 0 ? null : (
              <div key={label}>
                <div style={{ padding: "10px 16px 4px", fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", borderBottom: "1px solid var(--c-1e1e1e)" }}>{label}</div>
                {grpItems.map(item => (
                  <div key={item.id} style={{ ...styles.listItem, cursor: "pointer" }} onClick={() => { setSection(label); setSelected(item.id); setCharTab("detail"); }}>
                    ...
                    <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 500, fontSize: 14, color: "var(--c-b0a090)" }}>{item.name || item.title || "Untitled"}</div>
                  </div>
                ))}
              </div>
            ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", borderBottom: "2px dashed var(--c-2a2a2a)" }}>
              <input
                autoFocus
                value={searchQuery}
              ...
              <div style={{ display: "flex", gap: 4, marginTop: 8, flexWrap: "wrap" }}>
                {["All", "Characters", "Relationships", "Chapters", "Notes"].map(t => (
                  <button key={t} onClick={() => setSearchTypeFilter(t)}
                    style={{ background: searchTypeFilter === t ? "var(--c-ff1d8e)" : "none", color: searchTypeFilter === t ? "var(--c-0d0d0d)" : "#666", border: `1px solid ${searchTypeFilter === t ? "var(--c-ff1d8e)" : "#333"}`, borderRadius: 12, padding: "2px 9px", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "'Fredoka', sans-serif" }}>
                ...
                <div style={{ display: "flex", gap: 4, marginTop: 6, flexWrap: "wrap" }}>
                  {[null, "Draft", "Revising", "Final"].map(st => (
                    <button key={st || "any"} onClick={() => setSearchStatusFilter(st)}
                      style={{ background: searchStatusFilter === st ? "var(--c-0a203a)" : "none", color: searchStatusFilter === st ? "var(--c-7dd3fc)" : "#555", border: `1px solid ${searchStatusFilter === st ? "var(--c-7dd3fc)" : "var(--c-2a2a2a)"}`, borderRadius: 12, padding: "2px 9px", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "'Fredoka', sans-serif" }}>
                      {st || "Any status"}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              ...
              {searchQuery.trim() && (() => {
                ...
                return ["Characters", "Relationships", "Chapters", "Notes"].map(group => {
                  ...
                  return (
                    <div key={group}>
                      <div style={{ padding: "10px 16px 4px", fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", borderBottom: "1px solid var(--c-1e1e1e)" }}>{group}</div>
                      {gr.map(r => (
                        <div
                          key={r.id}
                        ...
                        >
                          ...
                          <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 500, fontSize: 14, color: selected === r.id ? "var(--c-7dd3fc)" : "var(--c-b0a090)" }}>{r.label}</div>
                        </div>
                      ))}
                    </div>
                  );
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {(currentStory.trash || []).length === 0 ? (
              ...
              <>
                <div style={{ padding: "10px 16px", borderBottom: "1px solid var(--c-1e1e1e)" }}>
                  <button style={{ background: "none", border: "2px solid var(--c-ff1d8e)", color: "var(--c-ff1d8e)", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, width: "100%", fontFamily: "'Fredoka', sans-serif" }} onClick={emptyTrash}>Empty Trash</button>
                </div>
                {[...(currentStory.trash || [])].reverse().map(t => {
                  const label = t.item.name || t.item.title || (t.collection === "relationships" ? relLabel(t.item) : "Untitled");
                  ...
                  return (
                    <div key={t.item.id} style={{ ...styles.listItem, cursor: "default", gap: 6 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 500, fontSize: 14, color: "var(--c-b0a090)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{label}</div>
                        <div style={{ fontSize: 10, color: "#555", textTransform: "capitalize" }}>{t.collection.replace(/s$/, "")} · {daysLeft}d left</div>
                      </div>
                      <button onClick={() => restoreTrashItem(t.item.id)} style={{ background: "none", border: "1px solid var(--c-7dd3fc)", color: "var(--c-7dd3fc)", padding: "2px 10px", borderRadius: 4, cursor: "pointer", fontSize: 11, fontFamily: "'Fredoka', sans-serif", flexShrink: 0 }}>Restore</button>
          ...
          <>
            {/* per-section search */}
            <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--c-1e1e1e)" }}>
              <input
            ...
            >
              {(() => {
                ...
                const matches = item => {
                  if (section === "Relationships") {
                    return relLabel(item).toLowerCase().includes(q);
                  }
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderTop: (isOver || dz === "before") ? "2px solid var(--c-ff1d8e)" : "2px solid transparent",
                        ...(dz === "after" ? { borderBottom: "2px solid var(--c-ff1d8e)" } : {}),
                        cursor: "grab",
                        position: "relative",
                    ...
                    >
                      ...
                      <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: isActive ? 700 : 500, fontSize: 15, color: isActive ? "var(--c-7dd3fc)" : "var(--c-b0a090)", flex: 1 }}>{label}</div>
                ...
                const folderRow = (f, depth) => {
                  ...
                  return (
                    <div key={"row-" + f.id}
                      ...
                      style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 10px", paddingLeft: 12 + depth * 18, cursor: "pointer", borderBottom: dz === "after" ? "2px solid var(--c-ff1d8e)" : "1px solid var(--c-1a1a1a)", background: dz === "inside" ? "var(--c-2a1428)" : "transparent", borderTop: dz === "before" ? "2px solid var(--c-ff1d8e)" : "2px solid transparent", boxShadow: dz === "inside" ? "inset 0 0 0 2px var(--c-ff1d8e)" : undefined }}
                    >
                      ...
                      {renamingItemFolderId === f.id ? (
                        ...
                        <span onDoubleClick={e => { e.stopPropagation(); setRenamingItemFolderId(f.id); }}
                          style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: "var(--c-c9b99a)", fontSize: 13, flex: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", background: "var(--c-0d0d0d)", borderBottom: "2px solid var(--c-1e1e1e)", overflowX: "auto", flexShrink: 0, minHeight: 38 }}>
          {tabs.map(tab => (
            <div key={tab.id} onClick={() => switchTab(tab.id)}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 14px", height: 38, cursor: "pointer", flexShrink: 0, maxWidth: 180, background: tab.id === activeTabId ? "var(--c-141414)" : "transparent", borderBottom: tab.id === activeTabId ? "2px solid var(--c-ff1d8e)" : "2px solid transparent", borderRight: "1px solid var(--c-1e1e1e)" }}>
              <span style={{ fontSize: 12, color: tab.id === activeTabId ? "var(--c-c9b99a)" : "#555", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{tab.label}</span>
          ...
          <button onClick={() => openNewTab()} title="New tab" style={{ padding: "0 14px", height: 38, background: "none", border: "none", borderRight: "1px solid var(--c-1e1e1e)", color: "#555", cursor: "pointer", fontSize: 20, lineHeight: 1, flexShrink: 0 }}>+</button>
          <div style={{ marginLeft: "auto", display: "flex", gap: 4, padding: "0 10px", flexShrink: 0, alignItems: "center" }}>
            <button onClick={undo} title="Undo (Ctrl+Z)" style={{ background: "none", border: "1px solid var(--c-2a2a2a)", color: "#555", cursor: "pointer", fontSize: 12, padding: "2px 8px", borderRadius: 4, fontFamily: "'Fredoka', sans-serif" }}>↩ Undo</button>
            <button onClick={redo} title="Redo (Ctrl+Y)" style={{ background: "none", border: "1px solid var(--c-2a2a2a)", color: "#555", cursor: "pointer", fontSize: 12, padding: "2px 8px", borderRadius: 4, fontFamily: "'Fredoka', sans-serif" }}>↪ Redo</button>
            <div style={{ width: 1, height: 20, background: "var(--c-2a2a2a)", margin: "0 4px" }} />
            {[['single','□'],['h2','⬜⬜'],['v2','▬▬'],['quad','⊞']].map(([layout, icon]) => (
              <button key={layout} onClick={() => changePaneLayout(layout)} title={layout}
                style={{ background: paneLayout === layout ? "var(--c-1a1a1a)" : "none", border: `1px solid ${paneLayout === layout ? "var(--c-ff1d8e)" : "var(--c-2a2a2a)"}`, color: paneLayout === layout ? "var(--c-ff1d8e)" : "#555", cursor: "pointer", fontSize: 11, padding: "2px 7px", borderRadius: 4 }}>
                {icon}
              </button>
            ))}
          </div>
          ...
          return (
            <div style={{ flex: 1, display: "grid", overflow: "hidden", ...gridStyle }}>
              {positions.map(pos => {
                ...
                return (
                  <div key={pos} style={{ display: "flex", flexDirection: "column", overflow: "hidden", border: isActive && paneLayout !== 'single' ? "2px solid var(--c-3a0a2e)" : "1px solid var(--c-1e1e1e)", position: "relative" }}>
                    ...
                    {paneLayout !== 'single' && (
                      <div
                        ...
                        style={{ display: "flex", alignItems: "center", gap: 6, height: 28, padding: "0 10px", background: isActive ? "var(--c-1a0a2e)" : "var(--c-0d0d0d)", borderBottom: "1px solid var(--c-1e1e1e)", cursor: isActive ? "grab" : "pointer", flexShrink: 0 }}>
                        {isActive && <div style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--c-ff1d8e)", flexShrink: 0 }} />}
                        <span style={{ fontSize: 11, color: isActive ? "var(--c-c9b99a)" : "#444", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, fontFamily: "'Fredoka', sans-serif" }}>{paneLabel(pos)}</span>
                      </div>
                    )}
        ...
        return (
          <div style={styles.overlay} onClick={() => setShowTraitsModal(false)}>
            <div style={{ ...styles.modalBox, width: 560 }} onClick={e => e.stopPropagation()}>
              <h3 style={{ fontFamily: "'Bangers', cursive", fontSize: 28, letterSpacing: "0.04em", color: "var(--c-ff1d8e)", marginBottom: 8, borderBottom: "2px dashed var(--c-3a0a2e)", paddingBottom: 12 }}>
                Character Traits
              </h3>
              ...
              />
              <div style={{ borderTop: "1px solid var(--c-2a2a2a)", paddingTop: 16 }}>
      ...
      {itemToDelete && (
        <div style={styles.overlay} onClick={() => setItemToDelete(null)}>
          <div style={styles.modalBox} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: "'Bangers', cursive", fontSize: 28, letterSpacing: "0.04em", color: "var(--c-ff1d8e)", marginBottom: 20, borderBottom: "2px dashed var(--c-3a0a2e)", paddingBottom: 12 }}>
              Delete {(itemToDelete.type === "char-sub" ? (itemToDelete.field === "outlineSubs" ? "outline entry" : "note") : itemToDelete.type).replace(/^./, ch => ch.toUpperCase())}
            </h3>
            <div style={{ fontSize: 14, color: "var(--c-c9b99a)", marginBottom: 24 }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              <button style={{ background: "var(--c-ff1d8e)", border: "2px solid var(--c-3a0a2e)", color: "var(--c-0d0d0d)", padding: "8px 16px", borderRadius: 4, cursor: "pointer", fontSize: 12 }} onClick={() => {
                ...
                } else if (itemToDelete.type === "char-sub") {
                  deleteCharSub(itemToDelete.charId, itemToDelete.field, itemToDelete.id);
                }
      ...
      {pdfPreview && (() => {
        const TYPE_META = {
          chapter:      { label: "Chapter",   color: "var(--c-7dd3fc)", icon: "📄" },
          character:    { label: "Character", color: "var(--c-ff1d8e)", icon: "👤" },
          note:         { label: "Note",      color: "var(--c-c9b99a)", icon: "📝" },
          relationship: { label: "Relation",  color: "var(--c-a78bfa)", icon: "↔"  },
        ...
        return (
          <div style={styles.overlay} onClick={() => { setPdfPreview(null); setSelectedPdfItems(new Set()); }}>
            <div style={{ ...styles.modalBox, width: 580, maxHeight: "85vh", display: "flex", flexDirection: "column" }} onClick={e => e.stopPropagation()}>
              ...
              {/* header */}
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14, borderBottom: "2px dashed var(--c-3a0a2e)", paddingBottom: 12, flexShrink: 0 }}>
                <h3 style={{ fontFamily: "'Bangers', cursive", fontSize: 26, letterSpacing: "0.04em", color: "var(--c-ff1d8e)", margin: 0 }}>PDF Import</h3>
                <span style={{ background: "var(--c-3a0a2e)", color: "var(--c-c050a0)", border: "1px solid var(--c-c050a0)", borderRadius: 4, padding: "2px 8px", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>{pdfPreview.format}</span>
              ...
              {/* item list */}
              <div style={{ flex: 1, overflowY: "auto", background: "var(--c-0d0d0d)", border: "1px solid var(--c-2a2a2a)", borderRadius: 8, padding: "6px 10px", marginBottom: 16 }}>
                ...
                {visible.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 4px", borderBottom: "1px solid var(--c-1a1a1a)" }}>
                      ...
                      <span style={{ fontSize: 13, color: "var(--c-c9b99a)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{label}</span>
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", flexWrap: "wrap", flexShrink: 0 }}>
                ...
                <button style={{ ...styles.addBtn, background: "var(--c-1a0a2e)", border: "2px solid var(--c-c050a0)", color: "var(--c-c050a0)" }} onClick={() => applyPDFImport(false)}>Add to "{currentStory?.title}" ({importCount})</button>
      ...
      {linkPicker && currentStory && (() => {
        ...
        const filtered = raw.filter(item => {
          if (linkPickerSection === "Relationships") {
            return !q || relLabel(item).toLowerCase().includes(q);
          }
        ...
        return (
          <div style={styles.overlay} onClick={() => setLinkPicker(null)}>
            <div style={{ ...styles.modalBox, width: 480 }} onClick={e => e.stopPropagation()}>
              <div style={{ fontFamily: "'Bangers', cursive", fontSize: 24, letterSpacing: "0.05em", color: "var(--c-7dd3fc)", marginBottom: 14, borderBottom: "2px dashed var(--c-2a2a2a)", paddingBottom: 12 }}>Insert Link</div>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
                {LINK_SECTIONS.map(s => (
                  <button key={s} onClick={() => setLinkPickerSection(s)}
                    style={{ background: linkPickerSection === s ? "var(--c-0a203a)" : "none", border: `1px solid ${linkPickerSection === s ? "var(--c-7dd3fc)" : "#333"}`, color: linkPickerSection === s ? "var(--c-7dd3fc)" : "#666", padding: "3px 12px", borderRadius: 20, cursor: "pointer", fontSize: 12, fontFamily: "'Fredoka', sans-serif", fontWeight: 700 }}>
                    {s}
                  </button>
                ))}
              </div>
              <div style={{ maxHeight: 260, overflowY: "auto", background: "var(--c-0d0d0d)", border: "1px solid var(--c-2a2a2a)", borderRadius: 8 }}>
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <div key={item.id} onClick={() => { linkPicker.onInsert(`[[${label}|${linkPickerSection}|${item.id}]]`); setLinkPicker(null); }}
                      style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid var(--c-1a1a1a)", fontFamily: "'Fredoka', sans-serif", fontSize: 14, color: "var(--c-c9b99a)", display: "flex", alignItems: "center", gap: 8 }}
                      onMouseEnter={e => e.currentTarget.style.background = "var(--c-1a1a2e)"}
      ...
      {focusChapterId && (() => {
        ...
        if (!ch) return null;
        return (
          <div style={{ position: "fixed", inset: 0, background: "var(--c-0d0d0d)", zIndex: 90, display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ width: "min(860px, 92vw)", display: "flex", alignItems: "center", gap: 12, padding: "18px 0 10px", flexShrink: 0 }}>
              <span style={{ fontFamily: "'Bangers', cursive", fontSize: 26, color: "var(--c-ff1d8e)", textShadow: "2px 2px 0 var(--c-3a0a2e)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ch.title || "Untitled"}</span>
              <span style={{ fontSize: 11, color: "#444" }}>Esc to exit</span>
              <button onClick={() => setFocusChapterId(null)} style={{ background: "none", border: "2px solid var(--c-2a2a2a)", color: "#888", width: 30, height: 30, borderRadius: 6, cursor: "pointer", fontSize: 15, fontWeight: 700, flexShrink: 0 }}>✕</button>
            </div>
...
// Fake Outlook chrome shown in disguise mode. Purely cosmetic; click the
// "Outlook" title (or press Ctrl+Shift+O) to exit; the ☾/☀ toggles light/dark.
// Segoe font comes from the global .disguise rule.
function OutlookRibbon({ onExit, dark, onToggleDark }) {
  const P = dark
    ? { bar: "#1b3a5c", search: "#3b3a39", searchText: "#c8c6c4", container: "#252423", border: "#3b3a39", tabsBg: "#292827", tabText: "#f3f2f1", tabActive: "#4cc2ff", btnBg: "#252423", btnText: "#f3f2f1", fileBg: "#2899f5", sep: "#3b3a39" }
    : { bar: "#0f6cbd", search: "#ffffff", searchText: "#605e5c", container: "#faf9f8", border: "#e1dfdd", tabsBg: "#f3f2f1", tabText: "#201f1e", tabActive: "#0f6cbd", btnBg: "#faf9f8", btnText: "#201f1e", fileBg: "#0f6cbd", sep: "#e1dfdd" };
  const SEGOE = { fontFamily: '"Segoe UI", Arial, sans-serif' };
  const tab = (label, active) => (
    <span style={{ padding: "6px 12px 5px", fontSize: 13, color: active ? P.tabActive : P.tabText, borderBottom: active ? `2px solid ${P.tabActive}` : "2px solid transparent", cursor: "default" }}>{label}</span>
  );
  const cmd = (icon, label) => (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, color: P.btnText, cursor: "default" }}>
      <span style={{ fontSize: 15 }}>{icon}</span>{label}
    </span>
  );
  const sep = <span style={{ width: 1, height: 26, background: P.sep, margin: "0 4px" }} />;
  return (
    <div style={{ ...SEGOE, position: "fixed", top: 0, left: 0, right: 0, height: 94, zIndex: 50, background: P.container, borderBottom: `1px solid ${P.border}`, display: "flex", flexDirection: "column", userSelect: "none" }}>
      {/* title bar */}
      <div style={{ height: 32, background: P.bar, display: "flex", alignItems: "center", padding: "0 12px", color: "#fff", flexShrink: 0 }}>
        <span title="Return to work" onClick={onExit} style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          <span style={{ fontSize: 15 }}>✉</span> Outlook
        </span>
        <div style={{ flex: 1, display: "flex", justifyContent: "center" }}>
          <div style={{ background: P.search, color: P.searchText, borderRadius: 4, fontSize: 12, padding: "3px 12px", width: "min(420px, 45%)", textAlign: "center" }}>Search</div>
        </div>
        <span title="Toggle dark mode" onClick={onToggleDark} style={{ fontSize: 14, cursor: "pointer", marginRight: 16 }}>{dark ? "☀" : "☾"}</span>
        <span style={{ display: "flex", gap: 16, fontSize: 12, opacity: 0.9 }}>&#8211; &#9633; &#10005;</span>
      </div>
      {/* ribbon tabs */}
      <div style={{ height: 30, background: P.tabsBg, borderBottom: `1px solid ${P.border}`, display: "flex", alignItems: "center", padding: "0 8px", flexShrink: 0 }}>
        <span style={{ padding: "6px 12px 5px", fontSize: 13, color: "#fff", background: P.fileBg, cursor: "default" }}>File</span>
        {tab("Home", true)}{tab("Send / Receive")}{tab("Folder")}{tab("View")}{tab("Help")}
      </div>
      {/* ribbon buttons */}
      <div style={{ flex: 1, background: P.container, display: "flex", alignItems: "center", gap: 12, padding: "0 14px" }}>
        {cmd("✉", "New Email")}{cmd("🗓", "New Items")}{sep}
        {cmd("🗑", "Delete")}{cmd("📥", "Archive")}{sep}
        {cmd("↩", "Reply")}{cmd("↪", "Reply All")}{cmd("➡", "Forward")}{sep}
        {cmd("📁", "Move")}{cmd("🏷", "Categorize")}
      </div>
    </div>
  );
}
